...
function renderSiralama() {
  ...
  const rows = playersOnly.map((user) => {
    ...
    const teamPoints = { "Beşiktaş": 0, "Galatasaray": 0, "Fenerbahçe": 0, "Trabzonspor": 0 };
    ...
    finishedMatches.forEach((match) => {
      ...
      // Kazanılan puanı, maçtaki ilgili takım(lar)a ekle
      if (teamPoints.hasOwnProperty(match.homeTeam)) {
        teamPoints[match.homeTeam] += points;
      }
      if (match.awayTeam !== match.homeTeam && teamPoints.hasOwnProperty(match.awayTeam)) {
        teamPoints[match.awayTeam] += points;
      }
    });

    return { uid: user.uid, username: user.username, P, TS, KB, Y, teamPoints };
  });
  ...
  if (teamStatsBody) {
    teamStatsBody.innerHTML = rows.map(r => `
      <tr>
        ...
        <td>${r.teamPoints["Beşiktaş"]}</td>
        <td>${r.teamPoints["Galatasaray"]}</td>
        <td>${r.teamPoints["Fenerbahçe"]}</td>
        <td>${r.teamPoints["Trabzonspor"]}</td>
      </tr>
    `).join("");
  }
}
